...
class get_data():
	...
	def read_label(self, dirpath):
		...
		for content in label_cont:
			...
			label_dict[cont_id] = np.array(cont_nu + add_num)
	...
	def data_generator(self):
		...
		while True:
			...
			inputs, outputs = self.get_batch(feats, labels)
			yield inputs, outputs


	# 将数据格式整理为能够被网络所接受的格式，被data_generator调用
	def get_batch(self, x, y, train=False, max_pred_len=50, input_length=100):
